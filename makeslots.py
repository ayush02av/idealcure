def MakeSlots():	
	...
	from database.models import Day, Slot
	import datetime
	...
	for day in days:
		slots += [slot for slot in Slot.objects.filter(DateAlloted=day, PatientTreatment=None)]
		Slot.objects.filter(DateAlloted=day, PatientTreatment=None).delete()
			...
			try:
				...
				for hour in range(5,9):
					try:
						...
						Slot(DateAlloted=dayToBeAlloted, PatientTreatment=None, TimeAlloted=datetime.time(12+hour,0,0)).save()
